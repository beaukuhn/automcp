...
const greeter_client_js_1 = require("../generated/greeter/greeter_client.js");
const calculator_client_js_1 = require("../generated/calculator/calculator_client.js");
async function testGreeter() {
    console.log("Testing Greeter service...");
    const greeter = new greeter_client_js_1.GreeterClient();
    let success = false;
    try {
        ...
        if (greeting === "Hello, World!") {
            ...
            console.error(`✗ Greeting verification failed. Expected "Hello, World!" but got "${greeting}"`);
        }
    }
    catch (error) {
...
async function testCalculator() {
    ...
    const calculator = new calculator_client_js_1.CalculatorClient();
    let success = true;
    try {
        ...
        if (product === "28") {
            ...
            console.error(`✗ Multiply verification failed. Expected "28" but got "${product}"`);
            success = false;
        }
    }
    catch (error) {
...
async function main() {
    ...
    if (serviceToTest === "greeter") {
        ...
        console.error(`Unknown service: ${serviceToTest}. Use "greeter" or "calculator".`);
        process.exit(1);
    }
